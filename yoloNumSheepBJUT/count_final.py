...
def parse_args():
    ...
    parser.add_argument('--base_path', type=str, default='D:/drone-overgrazing-detection/yoloNumSheepBJUT/',
                        help='模型路径')
    parser.add_argument('--img_path', type=str, default="D:/drone-overgrazing-detection/MediaGallery/2_20250623_adsdas/original/DJI_20250623085605_0008_V.JPG",
                        help='图片路径')
...
def process_image(args, output_folder, model):
    """图像处理增强版"""
    ...
    # 增强保存功能
    save_dir = args.base_path + "results/" + output_folder + "/"
    os.makedirs(save_dir, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_path = save_dir + output_folder + ".jpg"
    cv2.imwrite(save_path, annotated_image)
    print(f"结果已保存至: {save_path}")
    countFile = save_dir + "count.txt"
    # 将行数写入文件
...
def main():
    ...
    if input_source.lower().endswith(('.jpg', '.png', '.jpeg','.JPG')):
        print("处理单张图片")
        process_image(args, output_folder, model)
    else:
        print("不支持的输入格式")
    return "处理完成"
