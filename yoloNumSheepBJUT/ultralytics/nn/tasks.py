...
from .modules.LSKNet import LSKNet,LSKNET_Tiny
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c2 = ch[f]
            args = [c2 , *args]
        elif m is AutoBoxGraphAttention:
            c1 = ch[f]  # 获取输入通道数
            args = [c1, *args]  # 参数组合: [in_channels, num_heads, k_neighbors, ...]
            if verbose:
                print(f"AutoBoxGraphAttention args: {args}")
        elif m is GraphEnhancedDetect:
            c1 = ch[f]  # 自动获取输入通道数
            args = [c1] + args  # 重组参数列表
            if verbose:
                print(f"GraphEnhancedDetect层配置: 输入通道={c1}, 头数={args[1]}, 节点数={args[2]}")
        elif m is OtsuAttention:
            c2 = ch[f]  # 强制保持输入输出通道一致
            args = [ch[f]]  # 将输入通道数作为参数
        elif m is SimpleConcat:
            # 获取所有输入层的通道数（例如f=[1,13] -> c1=[128,512]）
            c1 = [ch[x] for x in f]
            # 将通道数列表和用户参数合并（例如c1=[128,512], args=[512] -> 最终参数[128,512,512]）
            args = [*c1, *args]
        elif m is AIFI:
            args = [ch[f], *args]
